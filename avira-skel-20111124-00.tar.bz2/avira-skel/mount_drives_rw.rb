		...
		d[5].each { |p|
			...
			begin
				...
				if p[2].strip != "mounted" && ( p[1] =~ /fat/ || p[1] =~ /ntfs/ ) 
					...
					#	else
					system("mount -o rw,noexec,nodev,nosuid,uid=0,gid=0,iocharset=utf8,noatime " + p[0].strip + " " + mountpoint) if p[1] =~ /fat/
					system("mount -t ntfs-3g -o rw,noexec,nodev,nosuid,uid=0,gid=0,utf8,noatime " + p[0].strip + " " + mountpoint) if p[1] =~ /ntfs/
					#	end
				end
			rescue
			end
