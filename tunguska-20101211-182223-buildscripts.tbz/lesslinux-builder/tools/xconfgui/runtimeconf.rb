...
LOCSTRINGS = {
	"de" => {
		"bank2010" => {
			...
			"pw_minlength" => "8",
			"password_title" => "COMPUTERBILD Sichere Bank 2010"
		},
	...
	IO.popen("lshw-static -quiet -short -class volume") { |i|
		while i.gets
			...
			if lcount > 1 && cols[3].to_s =~ /FAT/i
				fatvols.push(cols[1])
			elsif lcount > 1 && cols[3].to_s =~ /NTFS/i
				ntfsvols.push(cols[1])
...
def mount_crypt_container(container, pw)
	...
	unless system("mount -o nodev,nosuid,noexec /dev/mapper/lesslinux_crypt /home")
		# try to repair fs, preen should be safe enough
		system("/sbin/fsck.ext3 -p /dev/mapper/lesslinux_crypt") 
		unless system("mount -o nodev,nosuid,noexec /dev/mapper/lesslinux_crypt /home")
			system("cryptsetup luksClose lesslinux_crypt")
			system("losetup -d " + free_loop)
			system("umount /lesslinux/cryptpart")
...
def reinit_container(container, pw)
	puts("== Replace password hash ==")
	# puts pw
...
def reinit_fat_container(container, pw)
	unless File.exists?(container)
		return false, "Container file non-existent!"
	end
	teststr = String.new
...
if laxsudo == false || !crypt_container.nil?
	...
	assi.signal_connect('close')   { |w|
		...
		if d_pass.text.size > 0 && request_container_reinit(hexdigest) == true
			...
			reinit_container(crypt_container, d_pass.text)
			...
			unless fatcont.nil?
				reinit_fat_container(fatcont, d_pass.text) 
