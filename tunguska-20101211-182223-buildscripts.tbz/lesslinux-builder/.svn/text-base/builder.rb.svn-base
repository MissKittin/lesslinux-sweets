...
opts.on('--skip-files') { |i| @skiplist = i.strip }
opts.parse!
...
def install_stage02_package(p)
	...
	### Dir.chdir(@workdir)
	p.filecheck
	pb = PackageBuilder.new(@builddir, p.pkg_name, p.pkg_version, @dbh, @sqlite)
			...
			while (remaining_deps.size > 0 && rescount < 1_000_000 && circle_found == false && depends_on_circle == false) 
				...
				if already_resolved.include?(x)
					...
					@stage_two_objs.each { |i|
						rescount += 1
						if x == i.pkg_name
							circle_found = true if i.depends_on.include?(k.pkg_name)
...
def run_stage_three
	...
	stage_three_dirs.each { |d|
		if File.exists?(@builddir + d)
			puts sprintf("%015.4f", Time.now.to_f) + " error > Please remove the directories " + stage_three_dirs.join(" ") + " in " + @builddir + " first!"
			$stdout.flush
	...
	stage_three_objs.each { |i|
		...
		i.touchfiles
		i.scriptgen
		i.assemble_modules(@kernel_file)
		i.unpack
		### Dir.chdir(@workdir)
		i.install
		### Dir.chdir(@workdir)
		i.pkg_install( ["minimal","bin","config","localization","tzdata","uncategorized","skel"], ["de", "en"]) unless @full_image == true
		### Dir.chdir(@workdir)
	}
	...
	additional_deps.uniq.each { |a|
		...
		dep.install
		### Dir.chdir(@workdir)
		dep.pkg_install( ["minimal"], ["de", "en"]) unless @full_image == true
		### Dir.chdir(@workdir)
	}
