...
class AnyStage
	...
	def initialize (filename, sourcedir, builddir, unpriv, stage, dbh, unstable, sqlite, skiplist=nil)
		@buildfile = filename
		@srcdir = sourcedir
		@builddir = builddir
		@stage = stage
		...
		if File.exists?("scripts/" + @stage + ".unstable/" + @buildfile) && @unstable == true
	...
	attr_reader :pkg_name, :pkg_version, :xfile, :allowfail, :depends_on, :skiplist
	...
	def unpack
		...
		if File.exists?(@builddir + "/" + @stage + "/build/" + @pkg_name + "-" + @pkg_version + "/unpack.success") 
			...
			puts sprintf("%015.4f", Time.now.to_f) + " unpack > Unpacking " + @pkg_name + " " + @pkg_version + " successful"
			$stdout.flush
