...
class PackageBuilder
	...
	def initialize (basepath, pkg_name, pkg_version, dbh, sqlite)
		@basepath = basepath
		@pkg_name = pkg_name
	...
	def create_pkgdesc
		...
		#	" AND pkg_name='" + @pkg_name + "' AND stage='stage02' AND pkg_version='" + @pkg_version + "' ")
		sqlite_res = @sqlite.query("SELECT DISTINCT fname, fowner, fgroup, fmode, cleaned_type FROM llfiles WHERE ftype='f' " + 
			" AND pkg_name=? AND stage='stage02' AND pkg_version=? AND pkg_time=? ",
			[ SQLite3::Database.quote(@pkg_name), SQLite3::Database.quote(@pkg_version), @pkg_time.to_s ] )
		sqlite_res.each { |r|
