...
class AviraInfectedFile

	def initialize(path, mountpoint, uuid, infection, scanparams, fileout=nil, logdb=nil, tstamp=0, act=nil, fulllog=[])
		lang = ENV['LANGUAGE'][0..1]
		lang = ENV['LANG'][0..1] if lang.nil?
		lang = "en" if lang.nil?
		tlfile = "AviraInfectedFile.xml"
		tlfile = "/usr/share/lesslinux/engelmann-rescue/AviraInfectedFile.xml" if File.exists?("/usr/share/lesslinux/engelmann-rescue/AviraInfectedFile.xml")
	...
	def create_list_entry
		...
		nchk.tooltip_text = @tl.get_translation("act-none") # "Nichts tun"
		# desinfect
		dchk = Gtk::RadioButton.new(nchk, "")
		dchk.tooltip_text = @tl.get_translation("act-desinf") #"Desinfizieren" 
		# remove
		rchk = Gtk::RadioButton.new(nchk, "")
		rchk.tooltip_text = @tl.get_translation("act-delete") # "Löschen"
		ilabel.tooltip_text = @tl.get_translation("virus-link") # "In der Avira-Virendatenbank nachschlagen"
		ilabel.signal_connect("clicked") {
			system("su surfer firefox " + @tl.get_translation("virus-href") + @short_infection + " & ")
		}
		...
		IO.popen(scancl) { |l|
			while l.gets
				...
				unless pgbar.nil? || now - last_pulse < 0.1 
					last_pulse = now
					pgbar.text = @tl.get_translation("cleaning").gsub("FILENAME", @relpath)  
					pgbar.pulse
