...
def fill_combo(combo, button, pwinp)
	...
	begin
		...
		xcfg.elements.each("/chooser/connect") { |element| 
			...
			dom = element.attributes["domain"].to_s
			unless dom == ""
				dom = " -d \"" + dom + "\" "
			end
			cdepth = element.attributes["depth"].to_s
			cdepth = "24" if cdepth == ""
			xs = dom + " --ignore-certificate -a " + cdepth + " -f -u \"" + element.attributes["user"] + "\" \"" + element.attributes["host"] + "\"" 
			puts xs
			@execs.push xs
...
def connect(num, password)
	exec( "xfreerdp -p \"" + password + "\" " + @execs[num] )
end
...
go.signal_connect('clicked') { 
	connect(combo.active, pwinp.text)
}
