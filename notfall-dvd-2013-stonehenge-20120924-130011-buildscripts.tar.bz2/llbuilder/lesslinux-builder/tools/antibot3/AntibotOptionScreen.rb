...
class AntibotOptionScreen
	...
	def initialize
		...
		@elabel = Gtk::Label.new("Standardeinstellungen für erweiterte Gefahrenkategorien")
	...
	def show_options
		...
		frames = Array.new
		[ "Scanmethode", "Erweiterte Gefahrenkategorien", "Aktion bei Fund von Schädlingen" ].each { |s| frames.push(Gtk::Frame.new(s)) }
		...
		0.upto(ext_checks.size - 1) { |n|
			ext_checks[n].signal_connect('clicked') { 
				...
				@elabel.text = "Standardeinstellungen für erweiterte Gefahrenkategorien" if @extended_checks == @extended_defaults
				@elabel.text = "Eigene Einstellungen für erweiterte Gefahrenkategorien" unless @extended_checks == @extended_defaults
			}
		}
		frames[1].add exttab
