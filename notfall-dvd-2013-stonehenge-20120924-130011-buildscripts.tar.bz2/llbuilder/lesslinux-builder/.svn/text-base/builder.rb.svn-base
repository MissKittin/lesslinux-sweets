...
unless File.exist?(@builddir + "/lesslinux.sqlite")
	...
	@sqlite.execute("CREATE INDEX nameversion_idx ON llfiles (pkg_name ASC, pkg_version ASC) ")
else
	@sqlite = SQLite3::Database.new(@builddir + "/lesslinux.sqlite")
end
...
@stage_two_scripts.sort.each { |i| 
	@stage_two_objs.push(SecondStage.new(i, @srcdir, @builddir, @unpriv, "stage02", @dbh, @unstable, @sqlite, nil, @legacy, @allowfail))
}
