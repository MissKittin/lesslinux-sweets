	...
	def create_isoimage (kconfig)
		...
		squashdirs.each { |d|
			if system("test -f " + @builddir + "/stage03/squash/" + d + ".sqs")
				system("rsync -avP "  + @builddir + "/stage03/squash/" + d + ".sqs " + @builddir  + "/stage03/cdmaster/lesslinux/" )
				# FIXME! FIXME! FIXME!
				unless d == "usrbin" 
					mfile.write(d + ".sqs /" + d + "\n")
				else
		...
		[ "syslinux.cfg", "boot/isolinux/isolinux.cfg" ].each { |c|
			...
			kcfg.elements.each("kernels/kernel") { |k|
				...
				isolinux_cfg.write("APPEND initrd=/boot/isolinux/devs.img,/boot/isolinux/initram.img,/boot/isolinux/i" + kname + 
					...
					" quiet lang=de toram=999999999 " )
				if c == "syslinux.cfg"
					#isolinux_cfg.write(" skipservices=|luksmount|mountcdrom|loadata| ")
					isolinux_cfg.write(" skipservices=|mountcdrom|loadata|installer|xconfgui|roothash| ")
				else
					#isolinux_cfg.write(" skipservices=|luksmount|mountumass| ")
					isolinux_cfg.write(" ejectonumass=1 skipservices=|installer|xconfgui|roothash| ")
				end
		...
		[ "menu.c32", "vesamenu.c32", "isolinux.bin" ].each { |f|
		...
		system(" mkisofs -input-charset iso8859-15 -r -J -pad -o lesslinux.iso -b boot/isolinux/isolinux.bin " + 
			" -no-emul-boot -boot-info-table -boot-load-size 4 -c boot/isolinux/boot.cat -V lesslinux_234215 cdmaster")
		Dir.chdir(oldpwd)
	end
