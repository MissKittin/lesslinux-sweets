...
def mount(partinfo, rw)
	...
	if partinfo[1] == "fat"
		...
		if system("which ntfs-3g") && rw == true
			fstype = "-t ntfs-3g"
		else
			fstype = "-t ntfs"
		end
	...
	doc.elements.each("//node[@class='storage']") { |x|
		...
		x.elements.each("node[@class='disk']") { |element|
			...
			if element.attributes["id"] =~ /^cdrom/ 
				...
				mount_point = nil
				log_name = []
				element.elements.each("logicalname") { |l| log_name.push(l.text) unless l.text.strip =~ /^\/dev\/scd/ }
				state = "clear"
				if log_name.size > 1
			...
			element.elements.each("node[@class='volume']") { |n|
				...
				unless fstype == "swap" || is_extended == true
					...
					mount_point = nil
					log_name = []
					n.elements.each("logicalname") { |l| log_name.push(l.text) unless l.text.strip =~ /^\/dev\/scd/ }
					...
					n.elements.each("node[@class='volume']") { |m|
						begin
							...
							unless fstype == "swap"
								mount_point = nil
								log_name = []
								m.elements.each("logicalname") { |l| log_name.push(l.text) unless l.text.strip =~ /^\/dev\/scd/ }
								mount_point = log_name[1] if log_name.size > 1
		...
		if d[5].size > 0
			...
			d[5].each { |p|
				...
				unless p[2] == "mounted"
					...
					writeable.sensitive = false
					if p[3].to_s == "/lesslinux/cdrom"
						mount_button.label = extract_lang_string("pan_sysdrive")
						mount_button.sensitive = false
					elsif p[3].to_s == "/lesslinux/cryptswap"
						mount_button.sensitive = false
						mount_button.label = dev_details
					else
